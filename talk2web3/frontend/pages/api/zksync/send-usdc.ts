...
const abi = `[
  ...
  export default async function handler(req: NextApiRequest, res: NextApiResponse<ResponseData>) {
    ...
    const value = ethers.utils.parseUnits(_amount, 18)

    try {

        const contract = new ethers.Contract(USDC_CONTRACT, abi, sender_wallet);

        const transaction = await contract.transfer(address, value);

        console.log(transaction);

        console.log(`Sent ${ethers.utils.formatUnits(value, 18)} USDC to ${address}`);
        ...
        return res.status(200).json({ message: `Sent USDC to ${address}` })
            ...
            return res.status(500).json({ message: `Error sending USDC to ${address}` })
