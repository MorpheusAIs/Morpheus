...
The definition of a Compute provider is they have downloaded and run the Morpheus full node, connected their wallet and are ready to receive API / LLM / Compute requests from other Morpheus users. Compute provided can be calculated by including a signed transaction generated by the Smart Agent with the return of the output from the MOR transaction.
